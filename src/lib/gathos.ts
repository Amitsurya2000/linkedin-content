...
/**
 * Gathos rejects any dimension that is not between 512 and 2048 AND divisible
 * by 16 — a 422 the caller cannot recover from. The style presets are authored
 * in LinkedIn's native sizes (1080x1350, 1200x627), none of which satisfy that,
 * so every request is snapped here rather than in each call site.
 *
 * Rounding is to the NEAREST multiple of 16, which shifts the aspect ratio by
 * well under 1% — invisible next to the crop LinkedIn applies anyway.
 */
function snap(n: number, fallback: number): number {
  if (!Number.isFinite(n) || n <= 0) n = fallback;
  const clamped = Math.min(2048, Math.max(512, Math.round(n)));
...
export async function generateImage(
  ...
  const payload = {
    ...
    width: snap(opts.width ?? 1024, 1024),
    height: snap(opts.height ?? 1024, 1024),
