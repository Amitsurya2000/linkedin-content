 ...
 *
 * Kept in its own file, imported dynamically ONLY from the nodejs branch of
 * instrumentation.ts's register(): Next.js compiles instrumentation.ts for
 * BOTH the nodejs and edge runtimes, and the edge bundle has no fs/os/path —
 * a top-level import here (as this used to be, directly in instrumentation.ts)
 * makes every page 500 with "Module not found: Can't resolve 'fs'" before any
 * route handler runs.
 */
