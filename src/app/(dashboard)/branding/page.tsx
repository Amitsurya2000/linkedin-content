...
export default function BrandingPage() {
  ...
  return (
    <div className="w-full space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-[#1A1414] tracking-tight">Profile Kit</h1>
      ...
      <div className="rounded-2xl bg-white border border-[#F2DAD8] p-5 space-y-3">
        <div className="space-y-2">
          <Label className="text-sm font-medium text-[#1A1414]">Target role &amp; company type</Label>
          <Input
            value={targetRole}
            onChange={(e) => setTargetRole(e.target.value)}
            placeholder="e.g. Senior FP&A / analytics lead at a scaling fintech"
            className="bg-white border-[#F2DAD8] text-[#1A1414] placeholder:text-[#6B5B5A] rounded-xl h-10 text-sm"
          />
          <p className="text-xs text-[#6B5B5A]">Optional, but all three assets get sharper when they point at one target.</p>
        </div>
        <Button
          onClick={generate}
      ...
      {kit && (
        <>
          {/* ── Banner ── */}
          <section className="rounded-2xl bg-white border border-[#F2DAD8] overflow-hidden">
            ...
            <div className="px-5">
              ...
              <img src={bannerSrc} alt="Your LinkedIn banner" className="w-full rounded-xl border border-[#F2DAD8]" />
              <p className="text-[11px] text-[#6B5B5A] mt-1.5">
            ...
            <div className="p-5 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] uppercase tracking-wider text-[#6B5B5A] font-semibold w-14">Colour</span>
                {THEMES.map((t) => (
                  <button
                    key={t.k}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] uppercase tracking-wider text-[#6B5B5A] font-semibold w-14">Diagram</span>
                {VISUALS.map((v) => (
                  <button
                    key={v.k}
              ...
              <div className="rounded-xl bg-[#FDF3F2] border border-[#F2DAD8] p-4 space-y-1.5 text-xs">
                <p className="text-[#1A1414]"><span className="text-[#6B5B5A]">Tagline:</span> <span className="font-semibold">{kit.banner.tagline}</span></p>
                <p className="text-[#1A1414]"><span className="text-[#6B5B5A]">Pillars:</span> {kit.banner.pillars.join("  |  ")}</p>
                {kit.banner.visualConcept && (
                  <p className="text-[#1A1414]"><span className="text-[#6B5B5A]">Visual concept:</span> {kit.banner.visualConcept}</p>
                )}
                <p className="text-[#6B5B5A]">Left: {kit.banner.layout.left}</p>
                <p className="text-[#6B5B5A]">Center: {kit.banner.layout.center}</p>
                <p className="text-[#6B5B5A]">Right: {kit.banner.layout.right}</p>
              </div>
            </div>
          </section>

          {/* ── Headlines ── */}
          <section className="rounded-2xl bg-white border border-[#F2DAD8] p-5 space-y-3">
            <h2 className="text-[#1A1414] font-semibold">Headline options</h2>
            {kit.headlines.map((h, i) => (
              <div key={i} className={`rounded-xl border p-4 space-y-2 ${h.recommended ? "border-[#ED383B]/40 bg-[#ED383B]/5" : "border-[#F2DAD8]"}`}>
                ...
                <p className="text-xs text-[#6B5B5A]">{h.why}</p>
                {h.whenToUse && <p className="text-xs text-[#6B5B5A]"><span className="font-medium">Use when:</span> {h.whenToUse}</p>}
                <p className="text-[10px] text-[#6B5B5A]">{h.text.length} / 220 characters</p>
              </div>
            ))}
          </section>
          ...
          <section className="rounded-2xl bg-white border border-[#F2DAD8] p-5 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-[#1A1414] font-semibold">About section</h2>
          ...
          {(kit.alignment.length > 0 || kit.refine.length > 0) && (
            <section className="rounded-2xl bg-[#FDF3F2] border border-[#F2DAD8] p-5 space-y-3">
              ...
              {kit.refine.length > 0 && (
                <div>
                  <h3 className="text-xs uppercase tracking-wider font-bold text-[#6B5B5A] mb-1.5">What to refine next</h3>
                  <ul className="space-y-1">
                    {kit.refine.map((r, i) => (
                      <li key={i} className="text-xs text-[#6B5B5A]">• {r}</li>
                    ))}
                  </ul>
                </div>
              )}
