...
function Stat({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="rounded-xl bg-white/[.045] border border-white/10 p-4">
      <p className="text-[10px] uppercase tracking-wider text-white/62 font-bold">{label}</p>
      <p className="text-2xl font-bold text-white mt-0.5">{value}</p>
      {hint && <p className="text-[11px] text-white/62">{hint}</p>}
    </div>
  );
}
        ...
        {rows.map((r) => (
          <div key={r.key} className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              <span className="text-white font-medium">{r.key}{unit}</span>
            ...
            <div className="h-2 rounded-full bg-white/[.04] overflow-hidden">
...
export default function AnalyticsPage() {
  ...
  if (loading) {
    return <div className="flex items-center gap-3 text-sm text-white/62"><Loader2 className="w-4 h-4 animate-spin text-[#ED383B]" /> Loading…</div>;
  }
  ...
  return (
    <div className="w-full space-y-6">
        ...
        <div className="rounded-2xl bg-white/[.045] border border-white/10 p-5 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input value={form.label} onChange={(e) => set("label", e.target.value)} placeholder="Month-end close carousel" className="h-10 rounded-xl border-white/10 text-sm" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-white">Format</Label>
              <select value={form.postType} onChange={(e) => set("postType", e.target.value)} className="w-full h-10 px-3 rounded-xl bg-white/[.045] border border-white/10 text-sm text-white outline-none">
                {["text", "carousel", "article", "poll", "video", "image"].map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-white">Posted on</Label>
              <Input type="date" value={form.postedAt} onChange={(e) => set("postedAt", e.target.value)} className="h-10 rounded-xl border-white/10 text-sm" />
            </div>
            <div className="space-y-1.5 sm:col-span-2">
              <Label className="text-xs font-medium text-white">Hook archetype (optional)</Label>
              <Input value={form.hookCategory} onChange={(e) => set("hookCategory", e.target.value)} placeholder="A. Bold/counterintuitive claim" className="h-10 rounded-xl border-white/10 text-sm" />
            </div>
            {([["impressions", "Impressions"], ["reactions", "Reactions"], ["comments", "Comments"], ["reposts", "Reposts"], ["saves", "Saves"], ["profileViews", "Profile views"]] as const).map(([k, l]) => (
              <div key={k} className="space-y-1.5">
                <Label className="text-xs font-medium text-white">{l}</Label>
                <Input inputMode="numeric" value={form[k]} onChange={(e) => set(k, e.target.value)} placeholder="0" className="h-10 rounded-xl border-white/10 text-sm" />
              </div>
            ))}
          </div>
      ...
      {rows.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-white/10 bg-white/[.04] p-8 text-center">
          <TrendingUp className="w-7 h-7 text-white/62 mx-auto mb-2" />
          <p className="text-sm text-white font-medium">Nothing logged yet</p>
          <p className="text-xs text-white/62 mt-1">Log 5–10 posts and this page starts telling you which format, hook and day actually work for you.</p>
        </div>
      ) : (
        <>
          ...
          {insights?.findings.length ? (
            <div className="rounded-2xl bg-white/[.04] border border-white/10 p-5 space-y-2">
              <h3 className="text-sm font-semibold text-white flex items-center gap-1.5"><Lightbulb className="w-4 h-4 text-[#ED383B]" /> What the numbers say</h3>
              <ul className="space-y-1.5">
                {insights.findings.map((f, i) => <li key={i} className="text-xs text-white">• {f}</li>)}
              </ul>
            </div>
          ) : null}
          ...
          <div className="rounded-2xl bg-white/[.045] border border-white/10 overflow-hidden">
            <h3 className="text-sm font-semibold text-white p-4 pb-2">Logged posts</h3>
            <div className="divide-y divide-white/10">
              {rows.map((r) => (
                <div key={r.id} className="flex items-center justify-between gap-3 p-4">
                  ...
                  <button onClick={() => remove(r.id)} className="p-2 rounded-lg text-white/62 hover:text-[#ED383B] hover:bg-[#ED383B]/[.10] shrink-0">
