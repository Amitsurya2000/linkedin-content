...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; dot: string; text: string; bg: string }> = {
    ...
    pending: { label: "Pending", dot: "bg-white/[.08]", text: "text-white/62", bg: "bg-white/[.04]" },
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {!loading && (
            <>
              <span className="text-2xl font-bold text-white tabular-nums">{batches.length}</span>
              <span className="text-xs text-white/62 uppercase">Total</span>
            </>
          )}
          <Link href="/create">
      ...
      {loading && (
        <div className="space-y-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-14 bg-white/[.045] rounded-2xl border border-white/10 animate-pulse" />
          ))}
        </div>
      )}

      {!loading && batches.length === 0 && (
        <div className="rounded-2xl border border-dashed border-white/10 bg-white/[.045] flex flex-col items-center text-center py-20 space-y-4">
          <div className="w-14 h-14 rounded-2xl bg-white/[.04] border border-white/10 flex items-center justify-center">
            <Layers className="w-7 h-7 text-white/62" />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-white">No posts yet</h3>
            <p className="text-white/62 text-sm mt-1 max-w-xs">Your generated posts will appear here.</p>
          </div>
          <Link href="/create">
      ...
      {!loading && batches.length > 0 && (
        <div className="rounded-2xl bg-white/[.045] border border-white/10 overflow-hidden shadow-sm">
          <div className="hidden md:grid grid-cols-[2fr_80px_120px_120px_60px] gap-4 px-5 py-3 border-b border-white/10 bg-slate-50">
            <span className="text-xs font-medium text-white/62 uppercase tracking-wider">Business</span>
            <span className="text-xs font-medium text-white/62 uppercase tracking-wider">Posts</span>
            <span className="text-xs font-medium text-white/62 uppercase tracking-wider">Status</span>
            <span className="text-xs font-medium text-white/62 uppercase tracking-wider">Date</span>
            <span />
          </div>
              ...
              <div key={batch.id} className="grid grid-cols-1 md:grid-cols-[2fr_80px_120px_120px_60px] gap-4 items-center px-5 py-4 hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-white font-medium text-sm truncate">{batch.businessName}</p>
                    {batch.websiteUrl && <p className="text-white/62 text-xs truncate">{batch.websiteUrl}</p>}
                  </div>
                </div>
                <div className="text-sm text-white font-medium">
                  {batch.postsCount}<span className="text-white/62 font-normal ml-1 text-xs">post{batch.postsCount !== 1 ? "s" : ""}</span>
                </div>
