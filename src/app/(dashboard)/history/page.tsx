...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; dot: string; text: string; bg: string }> = {
    ...
    pending: { label: "Pending", dot: "bg-[#F5C5C7]", text: "text-[#6B6B6B]", bg: "bg-[#FCEBEC]" },
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#1A1A1A] tracking-tight">Posts</h1>
        ...
        <div className="flex items-center gap-4">
          {!loading && (
            <>
              <span className="text-2xl font-bold text-[#1A1A1A] tabular-nums">{batches.length}</span>
              <span className="text-xs text-[#6B6B6B] uppercase">Total</span>
            </>
          )}
          <Link href="/create">
            <Button className="bg-red-600 hover:bg-red-700 text-[#1A1A1A] gap-2 rounded-xl font-semibold cursor-pointer">
      ...
      {loading && (
        <div className="space-y-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-14 bg-white rounded-2xl border border-[#F5C5C7] animate-pulse" />
          ))}
        </div>
      )}

      {!loading && batches.length === 0 && (
        <div className="rounded-2xl border border-dashed border-[#F5C5C7] bg-white flex flex-col items-center text-center py-20 space-y-4">
          <div className="w-14 h-14 rounded-2xl bg-[#FCEBEC] border border-[#F5C5C7] flex items-center justify-center">
            <Layers className="w-7 h-7 text-[#6B6B6B]" />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-[#1A1A1A]">No posts yet</h3>
            <p className="text-[#6B6B6B] text-sm mt-1 max-w-xs">Your generated posts will appear here.</p>
          </div>
          <Link href="/create">
            <Button className="bg-red-600 hover:bg-red-700 text-[#1A1A1A] gap-2 rounded-xl cursor-pointer">
      ...
      {!loading && batches.length > 0 && (
        <div className="rounded-2xl bg-white border border-[#F5C5C7] overflow-hidden shadow-sm">
          <div className="hidden md:grid grid-cols-[2fr_80px_120px_120px_60px] gap-4 px-5 py-3 border-b border-[#F5C5C7] bg-slate-50">
            <span className="text-xs font-medium text-[#6B6B6B] uppercase tracking-wider">Business</span>
            <span className="text-xs font-medium text-[#6B6B6B] uppercase tracking-wider">Posts</span>
            <span className="text-xs font-medium text-[#6B6B6B] uppercase tracking-wider">Status</span>
            <span className="text-xs font-medium text-[#6B6B6B] uppercase tracking-wider">Date</span>
            <span />
          </div>
              ...
              <div key={batch.id} className="grid grid-cols-1 md:grid-cols-[2fr_80px_120px_120px_60px] gap-4 items-center px-5 py-4 hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[#1A1A1A] font-medium text-sm truncate">{batch.businessName}</p>
                    {batch.websiteUrl && <p className="text-[#6B6B6B] text-xs truncate">{batch.websiteUrl}</p>}
                  </div>
                </div>
                <div className="text-sm text-[#1A1A1A] font-medium">
                  {batch.postsCount}<span className="text-[#6B6B6B] font-normal ml-1 text-xs">post{batch.postsCount !== 1 ? "s" : ""}</span>
                </div>
